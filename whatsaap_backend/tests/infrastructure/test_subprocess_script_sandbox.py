...
"""
    result = await sandbox.run(script_source=script, input_payload={"message": {}}, secrets={})
...
async def test_script_without_handle_fails_cleanly() -> None:
    sandbox = await _verified_sandbox()
    result = await sandbox.run(script_source="x = 1", input_payload={"message": {}}, secrets={})
...
async def test_secrets_are_injected_as_env_vars() -> None:
    sandbox = await _verified_sandbox()
    script = """
import os

def handle(message):
    return {"business_data": {"key": os.environ.get("STRIPE_API_KEY")}}
"""
    result = await sandbox.run(
        script_source=script,
        input_payload={"message": {}},
        secrets={"STRIPE_API_KEY": "sk_live_test_value"},
    )

    assert result.ok is True
    assert result.business_data == {"key": "sk_live_test_value"}


async def test_secrets_cannot_override_path_or_home() -> None:
    """PATH/HOME are the sandbox's own controlled values — a secret named
    one of those must never be able to clobber them (see the env={**secrets,
    "PATH": ..., "HOME": ...} merge order in _run_unchecked)."""
    sandbox = await _verified_sandbox()
    script = """
import os

def handle(message):
    return {"business_data": {"path": os.environ.get("PATH")}}
"""
    result = await sandbox.run(
        script_source=script,
        input_payload={"message": {}},
        secrets={"PATH": "/malicious/path"},
    )

    assert result.ok is True
    assert result.business_data == {"path": "/usr/bin:/bin"}


async def test_secret_value_is_redacted_from_error_output() -> None:
    sandbox = await _verified_sandbox()
    script = """
import os

def handle(message):
    raise ValueError(os.environ["STRIPE_API_KEY"])
"""
    result = await sandbox.run(
        script_source=script,
        input_payload={"message": {}},
        secrets={"STRIPE_API_KEY": "sk_live_should_not_leak"},
    )

    assert result.ok is False
    assert result.error is not None
    assert "sk_live_should_not_leak" not in result.error
...
async def test_run_raises_when_not_verified() -> None:
    ...
    with pytest.raises(ScriptSandboxUnavailableError):
        await sandbox.run(
            script_source="def handle(message): return {}",
            input_payload={"message": {}},
            secrets={},
        )
