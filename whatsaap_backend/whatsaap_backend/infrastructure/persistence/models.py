...
class RuleExecutionModel(StandardColumns, Base):
    ...
    __table_args__ = (
        ...
        Index(
            "idx_rule_executions_tenant_session_creation",
            "tenant_id",
            "session_id",
            "creation_date",
            postgresql_where=text("expiration_date IS NULL"),
...
class SecretModel(StandardColumns, Base):
    """Ciphertext only. The plaintext value is never assigned to this ORM
    object as a Python attribute — SqlAlchemySecretsRepository writes it via
    a pgp_sym_encrypt() SQL expression and reads it via pgp_sym_decrypt(),
    so it never round-trips through ORM instance state or session identity
    map as plaintext."""

    __tablename__ = "secrets"
    __table_args__ = (
        UniqueConstraint("uuid_secrets", name="uk_secrets_uuid"),
        Index("idx_secrets_uuid", "uuid_secrets"),
        Index(
            "uk_secrets_tenant_name_active",
            "tenant_id",
            "name",
            unique=True,
            postgresql_where=text("expiration_date IS NULL"),
    ...
    pkid_secrets: Mapped[int] = mapped_column(BigInteger, Identity(always=True), primary_key=True)
    uuid_secrets: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), nullable=False, server_default=func.gen_random_uuid()
    )
    tenant_id: Mapped[str] = mapped_column(String(128), nullable=False)
