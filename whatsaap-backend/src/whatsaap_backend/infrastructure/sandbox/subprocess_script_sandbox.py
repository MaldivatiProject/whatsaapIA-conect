...
class SubprocessScriptSandbox:
    ...
    async def run(
        ...
        return await self._run_unchecked(script_source=script_source, input_payload=input_payload)
    ...
    async def _run_unchecked(
        ...
        with tempfile.TemporaryDirectory(prefix="rule-script-") as scratch_dir:
            process = await asyncio.create_subprocess_exec(
                ...
                # HOME is required by Chrome/chromedriver (profile/cache
                # dirs) — points at the disposable per-invocation scratch
                # dir, never the trusted process's real home. No other
                # secrets/credentials are passed.
                env={"PATH": "/usr/bin:/bin", "HOME": scratch_dir},
                cwd=scratch_dir,
