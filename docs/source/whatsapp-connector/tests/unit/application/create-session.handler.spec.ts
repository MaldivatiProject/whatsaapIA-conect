...
describe('CreateSessionHandler', () => {
  it('creates session and connects the socket', async () => {
    ...
    const result = await handler.execute(new CreateSessionCommand('my-session'));
  ...
  it('publishes SessionCreatedEvent', async () => {
    ...
    await handler.execute(new CreateSessionCommand('my-session'));
  ...
  it('throws SessionAlreadyExistsError when session exists', async () => {
    ...
    await handler.execute(new CreateSessionCommand('duplicate'));

    await expect(handler.execute(new CreateSessionCommand('duplicate'))).rejects.toThrow(
  ...
  it('throws SessionLimitReachedError when limit exceeded', async () => {
    ...
    await limitedHandler.execute(new CreateSessionCommand('session-1'));
    await limitedHandler.execute(new CreateSessionCommand('session-2'));

    await expect(limitedHandler.execute(new CreateSessionCommand('session-3'))).rejects.toThrow(
